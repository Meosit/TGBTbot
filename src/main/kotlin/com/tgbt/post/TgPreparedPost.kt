...
data class TgPreparedPost(
    ...
    val footerMarkdown: String = Setting.FOOTER_MD.str(),
...
suspend fun TgPreparedPost.sendToTelegram(
    targetChat: String,
    keyboardMarkup: InlineKeyboardMarkup? = null
): Message? {
    val usePhotoMode = Setting.USE_PHOTO_MODE.bool()
    val keyboardJson = keyboardMarkup?.let { BotJson.encodeToString(InlineKeyboardMarkup.serializer(), keyboardMarkup) }
    return when {
        usePhotoMode && canBeSendAsImageWithCaption -> TelegramClient.sendChatPhoto(
            targetChat,
