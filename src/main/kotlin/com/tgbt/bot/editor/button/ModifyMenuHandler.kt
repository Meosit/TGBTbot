...
object ModifyMenuHandler: ButtonMenuHandler("EDIT", "MODIFY") {
    ...
    private val editActions = mapOf<String, (UserSuggestion) -> UserSuggestion>(
        ...
        "laugh" to { it.copy(imageId = "https://sun6-21.userapi.com/impg/gqeV393Al3VYAL1PB0D1KWM41o48Jo3uBdniOQ/Ta4QytpxaAA.jpg?size=736x736&quality=95&sign=0d53e689938a9c1b0786a179e0127660&type=album") },
    ...
    override suspend fun handleButtonAction(
        ...
        return modifyPost(message, pressedBy, editAction)
    }

    suspend fun modifyWithCustomPic(message: Message, pressedBy: String, imageUrl: String): CallbackNotificationText {
        return if (imageUrl.isImageUrl()) {
            modifyPost(message, pressedBy) { it.copy(imageId = imageUrl) }
        } else {
            "❌Это не ссылка на изображение"
        }
    }

    private suspend fun modifyPost(
        message: Message,
        pressedBy: String,
        modifyAction: (UserSuggestion) -> UserSuggestion,
    ): String? {
        ...
        return if (suggestion?.editorChatId != null && suggestion.editorMessageId != null) {
            val updated = modifyAction(suggestion)
